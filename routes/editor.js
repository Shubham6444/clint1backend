...
// Configuration
// const ROOT_DIR = path.join(__dirname, '..', 'microservices');
const ROOT_DIR = path.resolve(__dirname, '..', 'database');
...
function validatePath(filePath, basePath = ROOT_DIR) {
    try {
...
// Route: Get file tree
router.get('/list', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}

    try {
        const tree = await walkDir(ROOT_DIR);
        res.json(tree);
    } catch (error) {
...
router.post('/read', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
...
router.post('/save', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
    ...
    try {
        ...
        // Create backup if file exists and is larger than 1KB
        const stats = await getFileStats(fullPath);
...
// Route: Upload files (fixed to handle single file properly)
router.post('/upload', upload.single('file'), async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
    ...
    try {
        ...
        res.json({
            ...
            path: path.relative(ROOT_DIR, finalPath)
        });
    } catch (error) {
...
router.post('/mkdir', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
...
router.post('/delete', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
...
router.post('/rename', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
...
router.post('/copy', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
    ...
    try {
        ...
        if (!validatePath(fullSourcePath) || !validatePath(fullDestPath)) {
            return res.status(403).json({ error: 'Invalid path' });
        }
...
router.post('/move', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
...
router.post('/search', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
    ...
    try {
        ...
        const searchDir = path.join(ROOT_DIR, searchPath);
...
router.post('/info', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
...
// Route: Download file (FIXED - removed problematic route pattern)
router.get('/download', async (req, res) => {const safePath = path.resolve(ROOT_DIR, req.query.path || '');
if (!safePath.startsWith(ROOT_DIR)) {
  return res.status(403).json({ error: 'Access denied' });
}
